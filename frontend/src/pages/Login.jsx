...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 relative overflow-hidden">
      ...
      >
        <motion.h1 variants={itemVariants} className="text-3xl font-normal text-slate-900 mb-6">Sign in</motion.h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.div variants={itemVariants}>
            <div className="flex justify-between items-center mb-1">
              ...
              <Link to="/forgot-password" className="text-sm text-indigo-600 hover:text-indigo-800 hover:underline">Forgot your password?</Link>
            </div>
            <motion.input
              whileFocus={{ scale: 1.01 }}
              type="password"
        ...
        <motion.div variants={itemVariants} className="mt-6">
          <p className="text-xs text-slate-600 leading-relaxed mb-6">
            By continuing, you agree to ServiceHub's <a href="#" className="text-indigo-600 hover:underline">Conditions of Use</a> and <a href="#" className="text-indigo-600 hover:underline">Privacy Notice</a>.
          </p>
          
          <div className="relative flex items-center justify-center">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-gray-200"></div></div>
            <div className="relative bg-white px-2 text-xs text-slate-500">Or continue with</div>
          </div>
          
          <motion.button
            whileHover={{ scale: 1.02, backgroundColor: '#f9fafb' }}
            whileTap={{ scale: 0.98 }}
            onClick={() => window.location.href = `${(import.meta.env.VITE_API_URL || 'https://servicehub-565o.onrender.com').replace(/\/$/, '')}/api/auth/google`}
            type="button"
      ...
      >
        <div className="relative flex items-center justify-center mb-4">
          ...
          <div className="relative bg-white px-3 text-xs text-gray-500">New to ServiceHub?</div>
        </div>
        
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          <Link to="/signup" className="block w-full text-center py-2 bg-gray-50 hover:bg-gray-100 text-slate-800 font-medium rounded shadow-sm border border-gray-300 transition-colors">
